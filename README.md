...
* http://dbwebb.se/nexus/current (try out the code)
...
Nexus is licenses according to MIT-license. 
...
Nexus uses external libraries for state of the art samples. Any external module can be replaced or
removed for less features but without disturbing the Nexus core functionality.
...
* Nexus always set timezone to UTC. All time values stored in database is in UTC. 
...
* Made `CMUser` a part of `CNexus` and `CObject`.
...
* Integrated `CViewContainer` in `CCGuestbook`, `CNexus`, `CObject`, and the theme handling.
* This makes `$nx->data` obsolete and to be removed in coming releases.
...
* `CNexus` creates and owns the databaseobject as `$nx->db`.
...
* Added session_start() in CNexus.
* Use default timezone as defined in site/config.php, set in CNexus::__construct()


v0.1.2 (2012-02-15) 

* One common baseclass for controllers and modules, CObject, holds access to CNexus through $this->
...
* Frontcontroller `CNexus::FronControllerRoute()` supporting varius url-constructs.
* A basic theme controller, `CNexus::ThemeEngineRender()`, with `functions.php`, `style.css` and template files.
